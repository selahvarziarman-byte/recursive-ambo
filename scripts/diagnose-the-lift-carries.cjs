...
const visibleText = (html) => unescapeHtml(html.replace(/<[^>]+>/g, ' ')).replace(/\s+/g, ' ');
{
  ...
  // the medial face is the finer ABAC·ABAD·ACAD; the lifted shape no longer holds the coarse edge AB–AC, so the face's edges are
  // medial in it and each hand offers a pair at the edge's own midpoint, or the act on the edge it descends from (in the record)
  note(`the lifted faces (§148 ruling 1 — the finer boundary): ${J(c.faces.map((f) => [f.name, f.kind, f.cornerCell]))}`);
  const medial = c.faces.find((f) => f.name === 'ABAC·ABAD·ACAD') ?? c.faces.find((f) => f.kind === 'born' && !f.cornerCell && f.name.split('·').length === 3);
  ...
  check('§6 ★ THE GEN-2 LIFT LISTS FOUR BORN FACES AND NO CORNER-CELL FACE (§148 ruling 1): the finer medial ABAC·ABAD·ACAD and the three corner triangles, all born; the residue\'s own sides (4 corners, opened through the midpoints) are not read as triangles',
    c.faces.length === 4 && c.faces.every((f) => f.kind === 'born' && !f.cornerCell) && ['ABAC·ABAD·ACAD', 'AB·ABAD·ABAC', 'ABAC·ACAD·AC', 'ABAD·AD·ACAD'].every((n) => c.faces.some((f) => f.name === n)), J(c.faces.map((f) => [f.name, f.kind, f.cornerCell])));
  const c1 = C1.concept;
...
console.log('\n----- §7 purity, the manifest, the mounts -----');
{
  ...
  check('§7 ★ THE SECTION reuses the Ambo\'s face blocks — FaceRecord and BornFaceRecord with here={null} and hands="words" — mounts NO inside panel in the column (C-10b: the drawing is the view\'s overlay on the sheet), and reaches no store', countOf(section, /<CastInsidePanel /g) === 0 && countOf(section, /<FaceRecord [^>]*here=\{null\} hands="words"/g) === 1 && countOf(section, /<BornFaceRecord [^>]*here=\{null\} hands="words"/g) === 1 && !/useGeometryStore/.test(section));
  check('§7 ★ THE DRAWING OPENS ON THE SHEET at its own size (C-10b): ManuscriptView mounts the Ambo\'s own CastInsidePanel (inline) ONCE, in a `data-lifted-drawing` overlay outside the card, for the picked corner of the selected lifted form', countOf(view, /<CastInsidePanel shape=\{liftedConcept\.record\} vertexId=\{liftedPicks\[selected\]\.vertex as string\} inline \/>/g) === 1 && countOf(view, /data-lifted-drawing=\{/g) === 1);
  const surface = readLf('src/components/MidpointSurface.tsx');
  const panels = readLf('src/components/Panels.tsx');
  check('§7 ★ THE AMBO\'S OWN MOUNTS AS C-10b PLACES THEM: SourceRecord mounts FaceRecord once with the site\'s edge as `here` (hands as acts) and BornFaceRecord no longer (the site NAMES each born face in a line); the face\'s HOME in Panels mounts FaceRecord and BornFaceRecord once each with here={null} (hands as acts); the Ambo\'s CastInsidePanel mount passes no `inline`', countOf(surface, /<FaceRecord /g) === 1 && countOf(surface, /<BornFaceRecord /g) === 0 && /<FaceRecord [^>]*here=\{site\.edge\.id\}/.test(surface) && /data-midpoint-born-face-at-site=/.test(surface) && countOf(panels, /<FaceRecord /g) === 1 && countOf(panels, /<BornFaceRecord /g) === 1 && /<BornFaceRecord [^>]*here=\{null\} \/>/.test(panels) && /<CastInsidePanel shape=\{shape\} vertexId=\{vertexId\} \/>/.test(surface));
}
