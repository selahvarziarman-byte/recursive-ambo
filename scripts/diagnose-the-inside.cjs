...
check('§5 ⛔ THE MODULE IS PURE OVER A CAST: castInside.ts imports only the type, the mold predicate from the loader and the foot\'s test (feet.ts, a leaf with no import of its own — C-12b: a foot\'s tuple is never a glyph); the drawing imports no store (it reads props and acts on nothing), nothing from manuscript, explore, three or the camera; no cast is written anywhere here',
  /import type \{ ConceptSpace \} from '\.\.\/types\/geometry';/.test(lib) && lib.includes("import { isMoldType } from './castLoader';") && lib.includes("import { isFootType } from './feet';") && (lib.match(/^import /gm) || []).length === 3 && !/^import /m.test(readLf('src/lib/feet.ts')) &&
