...
check('§5 ★★ SITED ON THE CANVAS, NOT THE INSPECTOR COLUMN (the designer\'s ruling): Workspace3D mounts `CastInsidePanel` for the selected vertex beside the R3F canvas (an overlay in the canvas frame, visible from every sidebar tab); Panels.tsx mounts no inside',
  ws.includes("import { CastInsidePanel } from './CastInsideDiagram';") && /\{selectedVertexId \? <CastInsidePanel shape=\{shape\} vertexId=\{selectedVertexId\} \/> : null\}/.test(ws) && !readLf('src/components/Panels.tsx').includes('CastInsidePanel'));
