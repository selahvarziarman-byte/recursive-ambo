  ...
  try {
    ...
    if (up) {
      ...
      for (const [w, h] of [[1689, 897], [1400, 900]]) {
        ...
        // ─── C-9 — THE BORN FACE at the eye ───
        const bf0 = br.bornFaces || []; const bf1 = (out.bornPair || {}).bornFaces || [];
        note(`C-9 the born faces at ABAC before a born pair: ${J(bf0.map((b) => ({ face: b.face, cells: b.cells, alike: b.alike, states: b.states, ground: b.ground, noNews: b.noNews, hands: b.hands, inPanel: b.inPanel, head: b.head })))} · after: ${J(bf1.map((b) => ({ face: b.face, alike: b.alike, states: b.states, news: b.news, noNews: b.noNews })))}`);
        check(`§7 [${w}×${h}] ★★ THE BORN FACES READ AT THE SITE (C-9, the designer's §125.1): at ABAC the two sources' faces are read through the resolver — the MEDIAL face (one cell, walked in the face's own direction) and the INTERIOR face (two cells, walked as the parent octahedron's, its other walk reading alike — one block and one line); each corner's SOLID part stated once and quietly (the ground), \`nothing added by a pair of yours yet\` before any born pair, Und's two hands ONCE per edge leading with WHERE — \`here, on AC–AB\` at this site, the descent to the seed edge named — the device never choosing`,
          bf0.length === 2 && bf0.some((b) => b.cells === '1' && /walked in the face's own direction/.test(b.head)) && bf0.some((b) => b.cells === '2' && b.alike === 'true' && b.alikeLine === 1 && /walked as the parent octahedron's/.test(b.head)) &&
            bf0.every((b) => b.states.length === 1 && b.states[0] === 'read' && b.ground === 3 && b.noNews === 3 && b.news.length === 0 && b.hands.length === 3 && b.hands.every((x) => /^(here, on [A-Z]+–[A-Z]+|at [A-Z]+, on [A-Z]+–[A-Z]+): a pair of yours · or at [A-Z]+, on [A-Z]–[A-Z]: an act on the edge it descends from$/.test(x)) && b.hands.filter((x) => /^here, on /.test(x)).length === 1),
          J(bf0));
        check(`§7 [${w}×${h}] ★★ THE EXTENSION MARKED AND ATTRIBUTED after the born pair (the news is the person's own): with the born pair standing the blocks keep their ground lines; where the pair closed a route a news line reads \`+ … returns … — through your pair x ↦ y at ABAC\` naming the pair through the spaces' labels, never a key; where it closed none the block still says \`nothing added by a pair of yours yet\` (the bound printed, not the case)`,
          bf1.length === 2 && bf1.every((b) => b.states.length >= 1 && b.ground === 3 * b.states.length) && bf1.every((b) => b.news.every((n) => /^\+ .+ returns (to itself|as .+) — through your pairs? .+ ↦ .+ at ABAC/.test(n) && !/[AB]:[A-Za-zΦ]/.test(n))) && (bf1.some((b) => b.news.length > 0) || (note('⚠ the leg\'s born pair closed no route at ABAC — the extension is empty here; the witness closes one on the same path'), true)),
          J(bf1));
        ...
        check(`§8 [${w}×${h}] ★★ THE LIFTED CORNER'S INSIDE at the eye: AB's row clicked mounts the Ambo's own inside panel INLINE in the card (origin derived, read on the record), its points drawn and at least one label wearing the person's \`≡\` — the transports still there on the lifted form; the panel's TOP inside the card's box once scrolled to (the reading scrolls — B-130 A.3; the panel is taller than the box, its height printed for the designer's eye)`,
          pab.insidePanel && pab.insidePanel.placement === 'inline' && pab.insidePanel.origin === 'derived' && pab.insidePanel.points >= 20 && pab.insidePanel.glyphed >= 1 && (pab.rows || []).some((x) => x.picked && x.text.startsWith('AB ·')) && sci.insidePanel && sci.insidePanel.topInside === true,
          J({ panel: pab.insidePanel, scrolledTopInside: sci.insidePanel ? sci.insidePanel.topInside : null, box: sci.insidePanel ? sci.insidePanel.box : null, scrollBox: sci.scrollBox }));
