  ...
  try {
    ...
    if (up) {
      ...
      for (const [w, h] of [[1689, 897], [1400, 900]]) {
        ...
        check(`§12 [${w}×${h}] ★★ THE WAY BACK (item 3): the workspace tab's Genealogy panel lists the session's shapes — \`Tetrahedron g0\` · \`Ambo Dissection Tetrahedron g1\` · \`Ambo Dissection Tetrahedron g2\` (the name ONCE at gen 2, item 2) — the current marked; g1 chosen is current — its row marked, reading gen 1's own census \`core:1 residue:4\` (and the arms that follow select gen 2's cuboctahedron, the store's own word) — then g2 chosen again (\`parent:2 core:1 residue:10\`)`,
          (gn.rows || []).length === 3 && /^Tetrahedron g0\b/.test(rowText(gn.rows, 0)) && /^Ambo Dissection Tetrahedron g1\b/.test(rowText(gn.rows, 1)) && /^Ambo Dissection Tetrahedron g2\b/.test(rowText(gn.rows, 2)) && gn.rows[2].current === true && gn.g1Buttons === 1 && (gn.rowsAtG1 || [])[1] && gn.rowsAtG1[1].current === true && /core:1 residue:4$/.test(rowText(gn.rowsAtG1, 1)) && gn.rowsAtG1.filter((r) => r.current).length === 1 && (gn.rowsAtG2 || [])[2] && gn.rowsAtG2[2].current === true && /parent:2 core:1 residue:10$/.test(rowText(gn.rowsAtG2, 2)) && gn.rowsAtG2.filter((r) => r.current).length === 1,
          J(gn));
