  ...
  try {
    ...
    if (up) {
      ...
      for (const [w, h] of [[1689, 897], [1400, 900]]) {
        ...
        check(`§19 [${w}×${h}] ★★ THE MEDIUM'S BLOCK IS ON THE SURFACE, in her words: the head counts (\`between A and B — … modes · … × … roles · … could be related · … related · … barred\`), the modes with IS, a gesture line, the passages through C and D with their readings, a one-line state — and NONE of the researcher's coinages printed`,
          !!mb && /^between A and B — \d+ modes? · \d+ × \d+ roles · \d+ could be related · \d+ related · \d+ barred$/.test(mb.head || '') && mb.modes.includes('IS') && mb.chosen === 'IS' && !!mb.gesture && mb.viewHeads.length === 2 && !!mb.stateLine && !/extent|lexicon|instance|verdict|centroid|discordance|vacuous|exhausted|pocket|unruled|coherent|closed|proposal|pushout|\\bpath\\b|edge [A-Z]{2}\\b/i.test(mb.text || ''),
          J(mb && { head: mb.head, modes: mb.modes, viewHeads: mb.viewHeads, stateLine: mb.stateLine, passages: mb.passages }));
        check(`§19 [${w}×${h}] ★★ THE PASSAGES READ WHERE THEY SIT: at AB with (i) on A–B, S1 on A–C and Q on C–B, C's passages read \`the face's — said between them and through C too\` for the one that comes to F13 ≡ r8, and \`only in C's light — through C it would read: … — no relating between A and B says so\` for the others; D — \`no passage yet\``,
          !!mb && mb.passages.includes('COMPOSED') && mb.passages.includes('LIGHT') && mb.passageTexts.some((s) => /the face's — said between them and through C too/.test(s)) && mb.passageTexts.some((s) => /only in C's light — through C it would read: .* — no relating between A and B says so/.test(s)) && mb.viewHeads.some((s) => /^D — no passage yet: nothing related on A–D or D–B$/.test(s)),
          J(mb && { passages: mb.passages, texts: mb.passageTexts.slice(0, 3), heads: mb.viewHeads }));
        check(`§19 [${w}×${h}] ★★ A MODE DECLARED AND A RELATING GIVEN BY TWO PICKS: \`carries\` typed and declared joins the modes; chosen, the gesture line reads \`… carries …\`; F2 then r3 (both unpaired) make \`<F2's name> carries <r3's name>\` — its own line with a hand; withdrawn, gone; IS chosen again`,
          !!md.declared && md.declared.modes.includes('carries') && !!md.chosen && md.chosen.chosen === 'carries' && /carries/.test(md.chosen.gesture || '') && !!md.related && md.related.relatings.length === 1 && / carries /.test(md.related.relatings[0]) && !!md.withdrawn && md.withdrawn.relatings.length === 0 && !!md.after && md.after.chosen === 'IS',
          J({ declared: md.declared && md.declared.modes, chosen: md.chosen && md.chosen.chosen, related: md.related && md.related.relatings, withdrawn: md.withdrawn && md.withdrawn.relatings.length, after: md.after && md.after.chosen }));
