  ...
  try {
    ...
    if (up) {
      ...
      for (const [w, h] of [[1689, 897], [1400, 900]]) {
        ...
        check(`§10 [${w}×${h}] ★★ TAKEN — THE WHOLE LINE, ONE HAND: F1 at A pointed, then F1 at A on the other side — the sentence \`taken — F1 ↦ F1 at A, and with it the whole line along A→AC→AB→A: F1 ↦ F1 · r2 ≡ F1 ↦ F1 · F1 ↦ F1\` (F1's class at AC named by this run's own pair on C–A); the line stands marked \`yours · a whole line along A→AC→AB→A: …\` with exactly ONE hand \`withdraw the line F1 ↦ F1\` (never a hand per pair); 6 chips marked yours (both sides); the head reads \`carries 3 role pairs on 1 whole line\``,
          dTaken && dTaken.lines === '1' && dTaken.pairs === '3' && /^taken — F1 ↦ F1 at A, and with it the whole line along A→AC→AB→A: F1 ↦ F1 · [^·]+ ≡ [^·]+ ↦ F1 · F1 ↦ F1$/.test(dTaken.taken || '') && (dTaken.lineBlocks || []).length === 1 && /^yours · a whole line along A→AC→AB→A: F1 ↦ F1 · [^·]+ ≡ [^·]+ ↦ F1 · F1 ↦ F1$/.test(dTaken.lineBlocks[0].words || '') && J(dTaken.lineBlocks[0].hands) === J(['withdraw the line F1 ↦ F1']) && dTaken.takenChips === 6 && /carries 3 role pairs on 1 whole line$/.test(dTaken.head || ''),
          J(dTaken));
