  ...
  try {
    ...
    if (up) {
      ...
      for (const [w, h] of [[1689, 897], [1400, 900]]) {
        ...
        check(`§19 [${w}×${h}] ★★ A MODE DECLARED AND A RELATING GIVEN BY TWO PICKS: \`carries\` typed and declared joins the modes; chosen, the gesture line reads \`… carries …\`; F13 then r8 make \`<F13's name> carries <r8's name>\` — its own line with a hand; withdrawn, gone; IS chosen again`,
