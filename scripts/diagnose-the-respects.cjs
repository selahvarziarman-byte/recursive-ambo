...
const AB1idx = 1;
check('§e ★★ THE DOOR RIDES IS-INSTANCES ONLY (MODES-1 · B6, D12 — its side spaces carry no respect type since B6; the record still holds the triad) AND ITS LINES NEVER READ ONE (fence 5): the side A·AB·AC read on the record with the respects and on the same record with them struck — the LINES byte-equal (a single-light respect glues nothing; the lines are drawn from the core and the roles), the side\'s space at AB carrying `⟨C⟩` with its tuple in the one and not the other (the space is M⁺, preserved through the door)',
...
check('§g ★★ THE COPY (the designer\'s §5; C-14h): unglued, the sentence reads `A and B together, as two — 24 roles · … words, 2 of them the views of the opposite corners, C\'s and D\'s — no tuple on them yet · … tuples · … marks`; the word-pairs box reads `no word translated yet — each word still its own side\'s, alike spellings included`; no person-facing `apart` or `foreign` in the surface\'s text, nor in its source outside comments',
  /^A and B together, as two — 24 roles · \d+ words, 2 of them the views of the opposite corners, C's and D's — no tuple on them yet · \d+ tuples · \d+ marks$/.test((elementsOf(h0, 'data-midpoint-sentence')[0] || {}).text || '') && ((elementsOf(h0, 'data-midpoint-word-pairs')[0] || {}).text || '').includes("no word translated yet — each word still its own side's, alike spellings included") && !/\bapart\b|\bforeign\b/.test(visible(h0)) && !/\bapart\b|\bforeign\b/.test(surfStrings),
