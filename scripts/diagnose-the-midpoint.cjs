...
check(`§4 ★★ THE UNGLUED STATE says so in words — never *nothing identified*, never *apart* (C-14 f, the designer's §5): \`A and B together, as two — ${zAB.roles} roles · ${zAB.words} words · ${zAB.tuples} tuples · 23 marks${zAB.clause}\` (§149: the space's count — the amalgam's 23 · 25 · 56 plus the feet's ${zAB.share.words} words and ${zAB.share.tuples} tuples, said beside it); both columns present (14 + 9 points, each side marked), NO line across the fold, the word rows with nothing translated said`,
  attrsOf(fresh, 'data-midpoint-state')[0] === 'unglued' && textsOf(fresh, 'data-midpoint-sentence')[0] === `${la} and ${lb} together, as two — ${zAB.roles} roles · ${zAB.words} words · ${zAB.tuples} tuples · 23 marks${zAB.clause}` && zAB.roles === 23 && zAB.words === 25 + zAB.share.words && zAB.tuples === 56 + zAB.share.tuples &&
