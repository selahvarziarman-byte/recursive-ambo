...
// C-14h — the designer's words (§187): the feet's share after the word count, the corners by their own labels in the unfolding's order, no ≡
const cornersViewWords = (share, names) => { if (share.words === 0) return ''; const owners = names.map((n) => `${n}'s`); const who = owners.length <= 1 ? `the view of the opposite corner, ${owners[0] || ''}` : `the views of the opposite corners, ${owners.slice(0, -1).join(', ')} and ${owners[owners.length - 1]}`; const them = share.words === 1 ? 'it' : 'them'; return share.tuples === 0 ? `${share.words} of them ${who} — no tuple on ${them} yet` : `${share.words} of them ${who} — ${share.tuples} ${share.tuples === 1 ? 'tuple' : 'tuples'} on ${them}`; };
const sizesAt = (shape, siteId, site) => { const r = spaceOf(shape, siteId); const c = spaceCounts(r.space); const order = [...new Set(site.sources.flatMap((s) => s.apexes))]; const names = [...order.filter((v) => r.feet.some((f) => f.corner === v)), ...r.feet.map((f) => f.corner).filter((v) => !order.includes(v))].map((v) => shape.vertices[v]?.data.label || v); const w = cornersViewWords(feetShareOf(r.feet), names); return { ...c, share: feetShareOf(r.feet), clause: w, wordsWords: w ? `${c.words} words, ${w}` : `${c.words} words` }; };
...
const zAB = sizesAt(ambo, siteAB.siteId, siteAB);
check(`§4 ★★ THE UNGLUED STATE says so in words — never *nothing identified*, never *apart* (C-14 f, the designer's §5): \`A and B together, as two — ${zAB.roles} roles · ${zAB.wordsWords} · ${zAB.tuples} tuples · 23 marks\` (§149: the space's count — the amalgam's 23 · 25 · 56 plus the feet's ${zAB.share.words} words and ${zAB.share.tuples} tuples, said beside it); both columns present (14 + 9 points, each side marked), NO line across the fold, the word rows with nothing translated said`,
  attrsOf(fresh, 'data-midpoint-state')[0] === 'unglued' && textsOf(fresh, 'data-midpoint-sentence')[0] === `${la} and ${lb} together, as two — ${zAB.roles} roles · ${zAB.wordsWords} · ${zAB.tuples} tuples · 23 marks` && zAB.roles === 23 && zAB.words === 25 + zAB.share.words && zAB.tuples === 56 + zAB.share.tuples &&
    attrsOf(fresh, 'data-midpoint-side').filter((s) => s === 'A').length === (forward ? 14 : 9) && attrsOf(fresh, 'data-midpoint-side').filter((s) => s === 'B').length === (forward ? 9 : 14) && countOf(fresh, 'data-midpoint-line') === 0 && visibleText(fresh).includes("no word translated yet — each word still its own side's, alike spellings included"),
...
check(`§4 ★★ THE TRACE, as description: the counts \`AB: 20 roles (14 + 9 − 3) · ${gAB.wordsWords} · ${gAB.tuples} tuples (4 both) · 20 marks\` (§149: the space's count — the amalgam's 20 · 22 · 52 plus the feet's ${gAB.share.words} words and ${gAB.share.tuples} tuples, said beside it; or with the corners the other way), \`what both confirm: 3 roles · 4 tuples · 3 marks\`, and per glued role \`F5 ≡ Φ7 · 7 tuples about it: both 1 · A 4 · B 2 · member_status has (A, B)\` (the sides named by the corners\' labels)`,
  (() => {
    ...
    const expectedCounts = forward ? `${lm}: 20 roles (14 + 9 − 3) · ${gAB.wordsWords} · ${gAB.tuples} tuples (4 both) · 20 marks` : `${lm}: 20 roles (9 + 14 − 3) · ${gAB.wordsWords} · ${gAB.tuples} tuples (4 both) · 20 marks`;
...
check('§4 ★★ THE TWO-HANDED CONTROL, rendered: the relational refusal on C–D names `sustains(r8, r0) does-not-hold in C · descends-from(Φ6, Φ1) holds in D` and offers THREE withdrawals — the attempt r0 ↦ Φ1, the prior r8 ↦ Φ6, the word pair sustains ↦ descends-from',
  (() => { const ws = attrsOf(cdHtml, 'data-midpoint-withdraw').filter((w) => !w.startsWith('role|r8|Φ6') || true); const inBox = (cdHtml.split('data-midpoint-refusal=')[1] || '').split('</div>')[0]; const boxWithdraws = attrsOf(inBox, 'data-midpoint-withdraw'); return countOf(cdHtml, 'data-midpoint-refusal') === 1 && attrsOf(cdHtml, 'data-midpoint-conflict')[0].startsWith(fwdCD ? 'sustains(r8, r0) does-not-hold in' : 'descends-from(Φ6, Φ1) holds in') && boxWithdraws.length === 3 && boxWithdraws.some((w) => w.startsWith('attempt|')) && boxWithdraws.some((w) => w.startsWith('role|')) && boxWithdraws.some((w) => w.startsWith('word|')) && ws.length >= 3; })(),
...
check('§4 ★★ THE MAPPED MIDPOINT\'S OWN DIAGRAM, ITS ORIGINS WRITTEN (C-7d item 1 · C-7f item 4): under (J₃, τ₃) the surface draws the glued space as ONE column — 20 points in the glue\'s order, the three glued roles marked, 4 tuples `both`; EVERY role and tuple carries its origin IN WORDS (`both` · `from A` · `from B`: 72 origin words — 20 roles + 52 tuples; 7 `both`), never colour alone (the tint stays as reinforcement); a translated word keeps its mark `sustains ≡ descends-from`, an untranslated alike spelling is shown plain (no `[A]` / `[B]` in the column\'s text), no `≡` glyph before a tuple\'s word; the unglued midpoint draws NO third column and says its own space is the two columns above',
  (() => {
    ...
    return attrsOf(glued, 'data-midpoint-own').includes('glued') && countOf(ownHtml, 'data-inside-point') === 20 && attrsOf(ownHtml, 'data-midpoint-own-role').filter((o) => o === 'both').length === 3 && attrsOf(ownHtml, 'data-midpoint-own-origin').filter((o) => o === 'both').length === 4 &&
      ...
      visibleText(glued).includes(`its own space, one column: 20 roles · ${gAB.wordsWords} · ${gAB.tuples} tuples · 20 marks ·`) && /every role and tuple says where it is from — both · from [AB] · from [AB]/.test(visibleText(glued)) &&
...
check('§5 ⛔ THE GLUE IS PURE OVER TWO CASTS AND THE PERSON\'S (J, τ): midpointGlue.ts imports only the types, the mold\'s join from the loader and the register\'s record/refusal/shared signature; no store, no component; the surface reaches the store only to act (the midpoint\'s five actions, MODES-1 · B5\'s one — `giveRelating`, the two-pick act in a chosen mode or barred; the medium\'s block reaches it for its own acts and reads — C-14 f\'s three — the triad, its withdraw, the attempt\'s — the face\'s one hand — `withdrawRolePair` again, in FaceRecord (C-5) — and the born face\'s one hand, `withdrawRolePair` again, in BornFaceRecord (C-9); three state reads in the chooser)',
  (gl.match(/^import /gm) || []).length === 3 && gl.includes("import { isMoldType, moldJoin } from './castLoader';") && gl.includes("import { recordOf, refusalOf, sharedSignature, type Conflict } from './jRegister';") && !/from '\.\.\/store|from '\.\.\/components/.test(gl) &&
    (surf.match(/useGeometryStore\(\(s\) => s\.(give|withdraw)/g) || []).length === 11 && surf.includes('useGeometryStore((s) => s.giveRelating)') && surf.includes('useGeometryStore((s) => s.giveTriad)') && surf.includes('useGeometryStore((s) => s.withdrawTriad)') && surf.includes('useGeometryStore((s) => s.withdrawTriadAttempt)') && !/updateSelected|\.cast\s*=/.test(surf) && !/J_CB|∘/.test(surf));
