...
check('§h defect 3, by construction: a foot is silent only when its map is empty AND the sorting finds NO path through that corner (`silent = f.map.size === 0 && !(view && view.paths.length > 0)`)', /const silent = f\.map\.size === 0 && !\(view && view\.paths\.length > 0\);/.test(msSrc) && /const sorting = useMemo\(\(\) => sortingOf\(shape, edgeBetween\(shape\.edges, site\.a, site\.b\), \{\}, \[\]\)/.test(msSrc));
if (vlShape) {
