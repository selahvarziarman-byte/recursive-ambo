...
const CAST_READERS = ['src/components/CastInsideDiagram.tsx', 'src/components/JRegisterPanel.tsx', 'src/components/MidpointSurface.tsx', 'src/components/Panels.tsx', 'src/components/VertexPacketEditor.tsx', 'src/lib/castInside.ts', 'src/lib/castLoader.ts', 'src/lib/faceReading.ts', 'src/lib/jRegister.ts', 'src/lib/midpointGlue.ts', 'src/store/geometryStore.ts'];
check('§4 ★ THE CAST HAS EXACTLY ITS ELEVEN (C-6c + C-6d (α) + (β) + C-7a + C-7b + C-5): the loader (src/lib/castLoader.ts), the writer (the packet editor), the reader (the card in Panels), the register that computes on the type (src/lib/jRegister.ts), the register\'s surface (src/components/JRegisterPanel.tsx), the INSIDE computed on the type (src/lib/castInside.ts) and its diagram on the canvas (src/components/CastInsideDiagram.tsx), the MIDPOINT computed on the type (src/lib/midpointGlue.ts) and its unfolded surface (src/components/MidpointSurface.tsx), the FACE computed on the type (src/lib/faceReading.ts — three corners\' casts as inputs to the residue and the guard), and the store — which reads the two corners\' casts ONLY to check an act at the act (the refusal is the act\'s, by construction) — no other file under the engine roots, the components or the store mentions `cast` or `ConceptSpace`',
