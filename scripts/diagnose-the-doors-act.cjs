...
check('§2 ★★ TAKEN — THE WHOLE LINE: F1 ↦ F1 at A takes with it the whole cycle along A→AC→AB→A (F1 ↦ F1 · Φ1 ≡ F1 ↦ F1 · F1 ≡ r0 ↦ F1 ≡ r0), and the transport records ONE entry per corner pair, one role pair each, the word pairs empty', /^taken — F1 ↦ F1 at A, and with it the whole line along A→AC→AB→A: F1 ↦ F1 · Φ1 ≡ F1 ↦ F1 · F1 ≡ r0 ↦ F1 ≡ r0$/.test(C1.eg['A taken'].words) && C1.eg['A taken'].transports.length === 3 && C1.eg['A taken'].transports.every((t) => t.roles.length === 1 && t.types.length === 0), J(C1.eg['A taken']));
...
check('§4 ★★ TAKEN — THE WHOLE LINE, ONE HAND: two lines read `yours · a whole line along A→AC→AB→A: F1 ↦ F1 · Φ1 ≡ F1 ↦ F1 · F1 ≡ r0 ↦ F1 ≡ r0` and F5\'s; exactly ONE `withdraw the line …` hand per line (2 hands for 6 pairs — never a hand per pair); the head reads `carries 6 role pairs on 2 whole lines`; the TAKEN sentence names the pointed pair and the whole line', countOf(takenHtml, /data-door-line="/g) === 2 && countOf(takenHtml, /data-door-withdraw-line="/g) === 2 && /yours · a whole line along A→AC→AB→A: F1 ↦ F1 · Φ1 ≡ F1 ↦ F1 · F1 ≡ r0 ↦ F1 ≡ r0/.test(takenHtml) && countOf(takenHtml, />withdraw the line F[15] ↦ F[15]</g) === 2 && /carries 6 role pairs on 2 whole lines/.test(takenHtml) && /data-door-taken="true"[^>]*>taken — F5 ↦ F5 at A, and with it the whole line along A→AC→AB→A: F5 ↦ F5 · /.test(takenHtml), J([countOf(takenHtml, /data-door-line="/g), countOf(takenHtml, /data-door-withdraw-line="/g)]));
