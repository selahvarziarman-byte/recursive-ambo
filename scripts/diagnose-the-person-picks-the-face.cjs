...
// `writtenFormModel.ts` (the context), `ManuscriptView.tsx` and
// `ManuscriptChrome.tsx` (the reused picker) carry THIS mandate's sanctioned
// edits — ratified above. Everything the contexts feed is frozen:
const guarded = [
  // `playgroundOperations.ts` LEFT this guard MID-BUILD for the engineer's
  // REFUSAL-ORDER re-cut (2026-07-13): its getDisabledReason sites now check
  // the FORM-level single-face gate BEFORE the selection-level face prompt —
  // order only, every string committed; ratified in section [t] above.
  // cut/collapse/dual/words themselves stay committed and correct; only WHICH
  // face reaches them and the ORDER of their refusals changed. Restore it
  // when the re-cut reaches HEAD.
...
check('the registry (cut/collapse untouched) · the engine · the certifiers · the combine door: byte-unchanged vs HEAD, CR-insensitively',
  dirty.length === 0);
