...
check('★ CLAUSE 4 — ZERO DRIFT AT BASELINE (import-closed at 5f3aecc, THE SMALL RUN; conformalAtom promoted at the APEX-LIFT union, 2026-08-02): ok === true · 48 files checked (47 + cornerCycleName.ts, the A-3b closure cure — the ONE corner-cycle composer frozen beside its frozen consumer) · drifted [] · missing [] · unlisted [] · nulled [] (any drift here means a hash was mis-taken — HARD FAIL)',
  base.ok === true && base.checked === 48 &&
...
check('COMPLETENESS: every .ts/.tsx under src/lib · src/playground · src/manuscript · src/types is classified — 48 FROZEN (the import closure: a frozen file is only as frozen as its dependencies), the rest NOT_FROZEN with a reason; `unlisted` non-empty would FAIL every witness',
  base.unlisted.length === 0 && base.frozen.length === 48);
...
note(`manifest: ${base.manifestPath} — 48 frozen (27 lib · 12 manuscript · 7 playground · 1 types · 1 data)`);
...
check('★ CLAUSE 3 — THE BITE: 48/48 one-character in-memory mutations FAIL the freeze (and each names exactly the mutated file)',
  bitesCaught === 48 && bitesExact === 48);
check('…and the CR-strip is not itself a hole: 48/48 CRLF re-expressions of the true content PASS (measured, not assumed)',
  crlfPasses === 48);
note(`bite: ${bitesCaught}/48 caught, ${bitesExact}/48 exact · CRLF: ${crlfPasses}/48 pass`);
