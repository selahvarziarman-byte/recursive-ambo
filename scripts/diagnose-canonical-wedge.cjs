...
// in-memory; NO leg of this diagnostic depends on HEAD being unfixed. The
// only `git show HEAD:` use left is §g's byte-guard on OTHER files — which is
// SUPPOSED to compare against HEAD, and stays honest across commits.)
//
...
check('everything else — ops · materializer · certifiers · classifiers · registry · gate · ledger — byte-unchanged vs HEAD, CR-insensitively',
  dirty.length === 0);
