...
check('§3 ★ THE VIEW hands the walk its room from the BUILT RECORD (seed + rows) and the carried record by the seed\'s own shape id (`cargoRoomOf(built.seed, shelfAncestors.get(built.seed.id) ?? [], built.rows, cellSurface, resolveAbsentLabel)`), null on a room with no record; the prop passed once',
  countOf(view, /const built = builtRecords\.find\(\(r\) => r\.key === domain\.key\) \?\? null;/g) === 1 && countOf(view, /cargoRoomOf\(built\.seed, shelfAncestors\.get\(built\.seed\.id\) \?\? \[\], built\.rows, cellSurface, resolveAbsentLabel\)/g) === 1 && countOf(view, /cargoRoom=\{exploreRoom\.cargoRoom\}/g) === 1);
