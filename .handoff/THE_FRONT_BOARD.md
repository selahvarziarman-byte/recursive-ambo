...
| seat | holding | state |
|---|---|---|
| **ARMAN** | ✅ **Δ73 given — the phase is definitional; the third corner on his clock in the other project.** Nothing owed | **the layer waits on U; the coder may run `B-3` whenever** |
| **CODER** (the ORDER seat) | ⇒ **`B-3` only** (Station B's four cuts); `C-4AMALGAM` WITHDRAWN — echo the withdrawal | **the layer is not theirs until the ground exists** |
| **DESIGNER (fourth)** | ⛔ **`C-3EDGE` SUSPENDED** (nothing on the layer until the ground); loop work live but unchartered | **at rest on the layer** |
| **RESEARCHER** | ⇒ **THE FRONT IS THEIRS: when U is cast — ONE FACE's MONODROMY, then the THEOREM, then the GROUND (an ADR)** — Arman's ruled order (Δ73). One line still owed: (a)(b)(c) confirmed | **the layer's live seat** |
...
| # | step | who | state |
|---|---|---|---|
| **C1** | **THE AMBO END** — DEFINITIONAL FIRST (Δ73): U cast → one face's MONODROMY → understood → THEOREM → the engine's GROUND → then the packet + the cast-taking + vertices-as-spaces, priced and built | researcher (monodromy, theorem, ground) → mothership (ratify) → Arman (the ground's word) → designer/coder | ⛔ **NO BUILD before the ground; C-4 withdrawn, C-3EDGE suspended** |
