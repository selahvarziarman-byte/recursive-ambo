...
Status: **drafted by the prompter/planner/auditor (lieutenant) for mothership ratification.** Not a Codex/Claude Code prompt, not campaign-plan amendment. It closes the Decision-D1 path-(a) schema-lift authorized by `PLATONIC_ENGINE_D1_LIFT_ENTRY_ORDER_HUB_LAYER_SOURCE_STATE_CAPSULE.md`.
...
This entry is binding by the human's ratification condition and must not be read away.
...
## 8. Sequencing after the lift

```txt
D1 lift closes (this memo, ratified)
...
Mothership: ____ ratify D1-lift closure / ____ return with changes.
Cleanup (human, native): one follow-up commit registering both npm-script lines in package.json.
Human (Arman): commits this memo to docs/governance/ on branch Claude-child.
```

On ratification, the lieutenant prepares the Station II entry (portability model) under a fresh mothership authorization, never self-initiated.
