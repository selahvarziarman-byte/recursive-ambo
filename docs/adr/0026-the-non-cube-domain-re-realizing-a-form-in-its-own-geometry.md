...
★ **The seam already exists in the substrate — and the engine does not refuse it, it draws its EUCLIDEAN SHADOW.** ⚠ *Correction (2026-08-26, mine — erratum at the head of §8): an earlier draft of this section cited a stale HEADER COMMENT in `apertureModel.ts` claiming `n<4⇒S³ · n>4⇒H³` and "S³/H³ … REFUSE." That classifier and that refusal were BOTH retired in July by a ratified researcher cut — **B.0 THE HONEST DOOR** (sealed `fab02d7e`): `buildAperture` (`apertureModel:1849`) draws EVERY sound form regardless of k, and the type `ApertureGeometry.kind` is `'E3' | 'cone'` (`:1272`) — S³/H³ are "geometries the substrate cannot hold."* ⇒ **So the engine does not diagnose-and-refuse; it DRAWS a curved form in the only model it has — the euclidean one — i.e. its euclidean SHADOW.** Measured at HEAD by the coder: B.4's Seifert–Weber domain builds a deck of 6 doors, `kind "cone"`, printing `cone edges: 6 × 450°` (a euclidean cone-manifold, more than a full turn per edge). ⇒ **B.4 is therefore not "build a refused transport" — it is: the engine already draws these forms in the WRONG model; §8 hands it the RIGHT one so the drawing matches the manifold that was built.** This ADR says what that model hand-off must mean, preserve, and prove.
...
**To re-realize a form in its own geometry is to replace the euclidean cells with S³ or H³ cells of the one size at which the edge-cycles close with no deficit — turning the cone-manifold into a *smooth* (locally homogeneous) spherical or hyperbolic manifold.** The engine **draws** the euclidean cone-manifold — the euclidean shadow, its edges marked as cone-lines; B.4 is the **cure** — the same edge-cycle, realized where its dihedral sum is exactly 2π, so the drawn form is the built one.
...
3. **The three target realizations:** the **dodecahedron → Seifert–Weber (H³)** and **→ Poincaré (S³)**, and the **lens → L(p,q) (S³)**. These already DRAW today, as euclidean shadows (`buildAperture`, `apertureModel:1849`); B.4 gives them the model that makes the drawing match the manifold.
...
- **The deficit → geometry reading** — NOT reused from the aperture (its `n`-classifier was retired in July with the refusal; `ApertureGeometry.kind` is now only `'E3' | 'cone'`). B.4's OWN deck-fit checker is the reading: `Σδ`-vs-`2π` per carried edge-cycle (the cube case `4×90°=360°`, the dodecahedron case `k×116.565°`). Seed-independent, and B.4's, not the aperture's.
...
⇒ **The argument descends one dimension unchanged (deficit determines geometry; solve the size for zero deficit; key the cycle on the carried complex). The machinery does not: the vertex-figure solver is replaced by an edge-figure solver. Build the second; reuse the carried edge-link census — the classifier is gone, and B.4 supplies its own deck-fit reading.**
...
- **There is no refusal to discharge — the aperture already DRAWS every sound form** (B.0 THE HONEST DOOR, `apertureModel:1874`). What B.4/§8 discharges is the WRONG MODEL: the engine draws a curved form's euclidean shadow (measured: Seifert–Weber → `6 × 450°`), and §8 hands the model that makes the drawn form the built one.
...
- **Chartered** 2026-08-26 (mothership letter `2026-08-26_1800…§8-IS-CHARTERED`), after B.4's first build landed: existence sealed, but *"from B.4 a person sees nothing"* — the engine draws the form's EUCLIDEAN SHADOW (measured: Seifert–Weber → a deck of 6 doors, `kind "cone"`, `cone edges: 6 × 450°`), never told apart from the manifold that was built. This section specifies the model hand-off that makes the drawn form the built one.
- ⚠ **ERRATUM (2026-08-26, researcher's — the correction that occasioned §8's re-grounding):** §0 and this section originally leaned on a stale HEADER COMMENT in `apertureModel.ts` (an `n<4⇒S³ · n>4⇒H³` classifier and an "S³/H³ … REFUSE"). **Both were retired in July by a ratified researcher cut — B.0 THE HONEST DOOR (sealed `fab02d7e`):** `buildAperture` (`apertureModel:1849`) draws every sound form regardless of k; the type `ApertureGeometry.kind` is `'E3' | 'cone'` (`:1272`). The classifier and refusal do not exist at HEAD. §8's SUBSTANCE (the three emitted fields, the trap, the control) is untouched — a comment was read as behaviour; the standing rule *audit the label against the emitting line* is why this was caught, and it is the reason §8's own trap (8.2) and control (8.3) key on measured lines, not comments.
- **§8 is the MECHANISM only** (the curved interior made *reachable and readable*). The **INHABITED WALK** — the person-driven walk-and-return, rung 3's acceptance, the curved analogue of rung 1's *"back where you started · N doors"* — stays deferred to its **own subsequent ADR** (number assigned at creation, not pinned here), chartered when *its* trigger fires (the render gate live and asking for the walk's certificate). §8 does not build habitation.
...
⇒ **The consumer (0004's ray-transport) is UNCHANGED.** §8 hands it the model it was promised; the door (`buildAperture`, `apertureModel:1849`) already draws — the coder swaps its euclidean transport for the in-model one, and the blocker is a TYPE, not a refusal: `DeckTransform` is 12 floats (`apertureModel:75`, a 3×3+translation over `V3`), and a Minkowski isometry is 4×4. *(NOT `apertureModel:446` — an earlier draft cited that; `:446` is inside `describeCandidate`, the corner-name reach. The stale `:446` was inherited from 0004, which carries the same fossil.)* The mechanism then shows the **see-through-doors recession** (0004 §3's mark) — the readable phenomenon, not a rate a person must be told to read.
